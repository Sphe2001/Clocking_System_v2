...
export default function VerifySupervisorEmail() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(
        `${domain}/api/auth/verifyotp/supervisor`,
        { otp },
  ...
  const handleResendOTP = async () => {
    try {
        const response = await axios.post(
          `${domain}/api/auth/resendotp/supervisor`,
          { otp },
          { withCredentials: true }
        );
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-l from-red-200 via-red-300 to-red-500">
      <Toaster />
